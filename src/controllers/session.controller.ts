...
class SessionController implements Controller {
  ...
  createUserSessionHandler = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
  ...
  deleteSessionHandler = async (
    ...
    return res.json(
      getSuccessResponse(`Session ID: ${sessionId} has been deactivated.`)
    );
  };
}
